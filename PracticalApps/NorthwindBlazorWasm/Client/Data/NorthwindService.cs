...
namespace NorthwindBlazorWasm.Client.Data
{
	public class NorthwindService : INorthwindService
	{
		...
		{
			HttpResponseMessage httpResponse = await http.PostAsJsonAsync<Customer>("api/customers", c);

			return await httpResponse.Content.ReadFromJsonAsync<Customer>();
		}
		...
		{
			HttpResponseMessage httpResponse = await http.DeleteAsync($"api/customers/{id}");
		}
	}
}
